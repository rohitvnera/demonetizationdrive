...
/**
 * Takes care of HTTP request/response pre-processing for login/logout and token check. Login can be performed on any URL,
 * logout only on specified {@link #logoutLink}. All the interaction with Spring Security should be performed via
 * {@link AuthenticationService}.
 * <p>
 * {@link SecurityContextHolder} is used here only for debug outputs. While this class is configured to be used by Spring
 * Security (configured filter on FORM_LOGIN_FILTER position), but it doesn't really depend on it at all.
 */
public final class TokenAuthenticationFilter extends GenericFilterBean
{
	...
	/**
	 * This is set in cases when we don't want to continue down the filter chain. This occurs for any
	 * {@link HttpServletResponse#SC_UNAUTHORIZED} and also for login or logout.
	 */
